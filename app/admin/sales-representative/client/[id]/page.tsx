...
export default function LeadDetailPage() {
  ...
  const {
    ...
    error,
    refetch
  } = useGetClientLeadByIdQuery(parseInt(leadId), {
    skip: !leadId,
  });

  // Discount code generation
  const [generateClientDiscountCode, { isLoading: isGenerating }] =
    useGenerateClientDiscountCodeMutation();

  const [updateLeadIntent] = useUpdateClientLeadIntentMutation();
  ...
  const filteredCPs = useMemo(() => {
    ...
    const mapped = crews.map((crew: any) => {
      ...
      return {
        ...
        name: `${crew.crew_member.first_name || ""} ${crew.crew_member.last_name || ""}`.trim() || "Unknown Member",
        image: imageUrl,
  ...
  const handleGenerateDiscount = async () => {
    ...
    try {
      const response = await generateClientDiscountCode({
        client_lead_id: parseInt(leadId),
        booking_id: lead?.booking_id || 0,
              ...
              <div className="flex flex-col gap-3 lg:gap-6 p-4 lg:p-9 lg:pb-6">
                ...
                {[["Base Price", basePrice], ["Editing Fee", editingCost], ["Additional Creatives", additionalCreatives]].map(([label, val]) => (
                  <div key={label as string} className="flex justify-between font-medium">
                    ...
                    <span className={`text-xs lg:text-base font-medium  ${isDark ? "text-white" : "text-black"}`}>${(val as number).toLocaleString()}</span>
                  </div>
                ))}
                {discountCodeValue && (
                  <div className="flex justify-between font-medium">
                    <span className="text-[#71717B] text-xs">Discount Code</span>
                    <span className={`text-xs lg:text-base font-medium  ${isDark ? "text-white" : "text-black"}`}>{discountCodeValue}</span>
                  </div>
                )}
                ...
                {referralInfo.code && (
                  <div className="flex justify-between font-medium">
                    ...
                    <span className={`text-xs lg:text-base font-medium  ${isDark ? "text-white" : "text-black"}`}>{referralInfo.code}</span>
                  </div>
                )}
