...
import { ArrowLeft, BadgeCheck, Check, Eye, MapPin, SquarePen } from "lucide-react";
...
const studioData = {
  ...
  description: "Branding and visual expert",
  hourlyRate: 85,
  overtimeRate: 100,
  minBooking: 2,
  bufferTiming: 30,
                ...
                <div className={`w-[67px] h-[67px] lg:w-36 lg:h-36 rounded-lg lg:rounded-xl overflow-hidden relative flex-shrink-0 border ${isDark ? "bg-[#222] border-white/5" : "bg-gray-100 border-gray-200"}`}>
                  {studioData.images.length > 0 ? (
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center text-3xl font-bold ${isDark ? "text-[#444] bg-[#222]" : "text-gray-400 bg-gray-100"}`}>
                      {studioData.name.split(' ').map((n: string) => n[0]).join('').toUpperCase().substring(0, 2)}
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-start w-full">
                  <div>
                    <div className="flex items-center gap-2 mb-2.5">
                      <h1 className={`text-lg lg:text-2xl font-medium ${isDark ? "text-white" : "text-black"}`}>{studioData.name}</h1>
                    ...
                    <div className={`flex items-center gap-1 text-xs mb-2 lg:mb-5 ${isDark ? "text-[#C2C2C2]" : "text-gray-600"}`}>
                      ...
                      <span>{studioData.location || "N/A"}</span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {studioData.supportedTypes.map((type) => (
            ...
            <div className={`${isDark ? "bg-[#111] border-[#333]" : "bg-[#F0F0F0] border-[#E3E3E3]"} rounded-2xl`}>
              ...
              <div className="p-4 lg:p-9">
                {activeTab === "Overview" ? (
                  ...
                  <>
                    <StudioGallery items={studioData.images} isDark={isDark} />
                  </>
                )}
              </div>
            </div>
