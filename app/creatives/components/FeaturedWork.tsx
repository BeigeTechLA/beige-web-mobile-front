...
export const FeaturedWork = ({ items = [], autoplay = false }: FeaturedWorkProps) => {
...
const Card = ({ item, index, total, progress, onSelect, windowWidth }: any) => {
  ...
  const getSpacing = () => {
    // Desktop: 500px width + 40 gap
    if (windowWidth >= 1024) return 540;
    // Tablet: 320 width + 30 gap
    if (windowWidth >= 768) return 350;
    // Mobile: 250 width + 20 gap
    return 270;
  };
  ...
  // Convex Curve: Center is 0, sides recede to -300
  const z = useTransform(offset, (o) => Math.abs(o) * -300);

  // Angle cards slightly outward for the convex bulge
  ...
  return (
    <motion.div
      ...
      onClick={onSelect}
      className="absolute rounded-[20px] overflow-hidden border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,0.8)] cursor-pointer group w-[250px] h-[300px] md:w-[320px] md:h-[450px] xl:w-[500px] lg:h-[650px]"
    >
      <div className="absolute inset-0 transition-transform duration-700 ease-out group-hover:scale-105">
        <Image
          ...
          sizes="(max-width: 768px) 250px, (max-width: 1024px) 320px, 600px"
        />
      </div>
