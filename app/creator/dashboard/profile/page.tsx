...
import {
  formatCreatorRoles,
  normalizeCreatorRoleIds,
} from "@/lib/creatorRoles";
...
const StatBox = ({
  ...
  <div
    className={`w-full flex flex-col items-center justify-center last:border-r-0 px-4 py-2 ${isDark ? "border-r border-white/5" : "border-r border-[#F5EBDA]"}`}
  >
    <p
      className={`lg:text-lg font-bold ${isDark ? "text-white" : "text-[#14171F]"}`}
    >
      {value}
    </p>
    <p
      className={`text-xs lg:text-sm ${isDark ? "text-white/40" : "text-[#677084]"}`}
    >
      {sublabel}
    </p>
  </div>
...
const SocialButton = ({
  ...
  <button
    onClick={onClick}
    className={`flex items-center gap-2 px-4 py-2 border rounded-lg lg:rounded-xl transition-colors ${
      isDark
  ...
  >
    ...
    <span
      className={`text-xs font-medium ${isDark ? "text-white/80" : "text-[#8C8C8C]"}`}
    >
      {label}
    </span>
  </button>
...
const getEmbedUrl = (url: string) => {
  ...
  const ytMatch = fullUrl.match(
    /(?:youtu\.be\/|youtube\.com(?:\/embed\/|\/shorts\/|\/v\/|\/watch\?(?:[^#]*&)?v=|\/ytscreeningroom\?(?:[^#]*&)?v=))([\w-]{11})/i,
  );
  if (ytMatch) {
  ...
  // Vimeo
  const vimeoMatch = fullUrl.match(
...
const SectionHeader = ({
  ...
  <div className="flex items-center justify-between mb-4 lg:mb-8">
    <h2
      className={`text-lg lg:text-xl font-bold tracking-tight ${isDark ? "text-white" : "text-black"}`}
    >
      {title}
    </h2>
    <button
...
const TabEmptyState = ({
  ...
  <div
    className={`border rounded-lg lg:rounded-2xl h-[500px] flex items-center justify-center relative overflow-hidden ${
      isDark ? "bg-[#0A0A0A] border-white/5" : "bg-neutral-50 border-black/5"
    }`}
  >
        ...
        <div
          ...
          className={`h-full border-r last:border-r-0 ${
            isDark
              ...
              : "border-black/[0.02] bg-black/[0.005]"
          }`}
        />
      ))}
    </div>
    ...
    <div className="relative z-10 flex flex-col items-center text-center max-w-xl px-6">
      <h2
        className={`text-2xl font-bold mb-3 tracking-tight ${isDark ? "text-white" : "text-black"}`}
      >
        {title}
      </h2>
      <p
        className={`text-sm mb-10 leading-relaxed max-w-md ${isDark ? "text-white/40" : "text-black/40"}`}
      >
        {description}
      </p>
      ...
      <button
        onClick={onClick}
        className={`font-bold px-10 py-3.5 rounded-lg lg:rounded-2xl transition-all active:scale-95 shadow-xl ${
          isDark
      ...
      <p
        className={`text-xs mt-6 font-medium uppercase tracking-widest ${isDark ? "text-white/30" : "text-black/30"}`}
      >
        {footerText}
      </p>
    </div>
  ...
  >
    <div className="flex items-center gap-4">
      ...
      <div className="overflow-hidden">
        <p
          className={`text-sm font-medium truncate max-w-[150px] md:max-w-[300px] ${isDark ? "text-white" : "text-black"}`}
        >
          {file.name}
        </p>
        <p
          className={`text-xs uppercase tracking-widest ${isDark ? "text-white/40" : "text-black/40"}`}
        >
...
const normalizeProfileEquipment = (value: unknown) => {
  const parsed = parseMaybeJsonDeep(value);
  ...
  return parsed
    .map((item) => {
      if (item && typeof item === "object" && !Array.isArray(item)) {
        const equipment = item as Record<string, any>;

        const nestedEquipment =
          equipment.equipment && typeof equipment.equipment === "object"
            ? equipment.equipment
            : null;
...
export default function ProfilePage() {
  ...
  const [editingFeaturedWork, setEditingFeaturedWork] = useState<any | null>(
    null,
  );
  const [isPageLoading, setIsPageLoading] = useState(false);
  const [isGoogleOnboardingOpen, setIsGoogleOnboardingOpen] = useState(false);
  const [googleOnboardingData, setGoogleOnboardingData] =
    useState<GoogleOnboardingData | null>(null);
  ...
  const [equipmentIds, setEquipmentIds] = useState<Array<string | number>>([]);
  ...
  useEffect(() => {
    ...
    const directEquipment = normalizeProfileEquipment(profile?.equipment);

    const ownershipEquipment = normalizeProfileEquipment(
      profile?.equipment_ownership,
    );

    const equipmentList =
      directEquipment.length > 0 ? directEquipment : ownershipEquipment;

    const ids = equipmentList.map((item) => item.id);
      ...
      const names = await Promise.all(
        equipmentList.map(async (item) => {
          ...
          try {
            ...
            const equipment = Array.isArray(payload) ? payload[0] : payload;
  ...
  }, [profile?.equipment, profile?.equipment_ownership]);
  ...
  useEffect(() => {
    ...
    const storedOnboarding =
      typeof window !== "undefined"
        ? sessionStorage.getItem(GOOGLE_ONBOARDING_STORAGE_KEY)
        : null;
    if (!storedOnboarding) {
  ...
  const profilePhotoFile = profile.crew_member_files?.find(
    (f: any) => f.file_type === "profile_photo",
  );
  ...
  const featuredWorks =
    profile.crew_member_files?.filter(
  ...
  const showOnboardingBanner =
    hasProfileLoaded && hasOnboardingStatus && missingCount > 0;
  ...
  const handleSaveSkills = async () => {
    ...
    const skillIds = profile.skills.map((s: any) =>
  ...
  const handleSaveEquipment = async () => {
    ...
    const payload = {
      ...
      equipment_ownership: equipmentIds.map(String),
    };
  ...
  const handleSaveFeaturedWork = async (data: any) => {
    ...
    try {
      ...
      toast.success(
        editingFeaturedWork ? "Featured work updated" : "Featured work added",
      );
    } catch (err) {
  ...
  const getFeaturedWorkModalItem = (project: any) => {
    ...
    return {
      ...
      previews: images
        .map((image: any) =>
          image?.file_path ? `${S3_BASE_URL}${image.file_path}` : "",
        )
        .filter(Boolean),
  ...
  const handleAddPortfolioLinks = async (updatedLinks: any[]) => {
    ...
    try {
      ...
      const existingLinks =
        profile.crew_member_files?.filter((f: any) => f.file_type === "link") ||
        [];

      const newLinks = updatedLinks.filter(
        (l) => !existingLinks.find((ex) => ex.crew_files_id === l.id),
      );
      const updatedExistingLinks = updatedLinks.filter((l) =>
        existingLinks.find((ex) => ex.crew_files_id === l.id),
      );
      ...
      const editPromises = updatedExistingLinks.map(async (l) => {
        ...
        if (
          existing &&
          (existing.file_path !== l.url || existing.tag !== l.platform)
        ) {
      ...
      const deletedPromises = existingLinks
        ...
        .map((ex) =>
          DeleteProfileFile(ex.crew_files_id, {
            crew_member_id: parseInt(crewMemberId),
          }),
        );
    ...
    <>
        ...
        >
          <div className="flex items-center justify-between gap-6">
            ...
            <div className="flex-1 min-w-0 mr-4">
              ...
              <div className="mt-2">
                <div
                  className={`w-full h-1.5 rounded-full overflow-hidden ${isDark ? "bg-black/30" : "bg-black/10"}`}
                >
                  <div
                ...
                <div
                  className={`mt-1.5 text-xs font-semibold sm:text-[13px] ${isDark ? "text-white/55" : "text-black/55"}`}
                >
                  {completedCount}/{totalRequired} Required Fields |{" "}
                  {missingCount} Remaining | Complete all required details to
                  become eligible for shoot assignments
                </div>
              </div>
            </div>
      ...
      <div className="overflow-hidden p-4 lg:p-6 lg:px-10 lg:py-9 space-y-6">
        <div className="mx-auto space-y-4 lg:space-y-8">
          {/* TOP PROFILE CARD */}
          <div
            className={`border rounded-lg lg:rounded-xl p-4 lg:p-8 flex flex-col lg:flex-row gap-8 ${isDark ? "bg-[#111111] border-white/5" : "bg-white border-[#E5E5E5]"}`}
          >
            <div className="flex-1 space-y-4 lg:space-y-6">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  <div
                    className={`group relative w-15 h-15 lg:w-20 lg:h-20 rounded-full border-2 border-[#E8D1AB] shrink-0 ${isDark ? "bg-zinc-800" : "bg-zinc-100"}`}
                  >
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h1
                        className={`text-lg lg:text-2xl font-bold ${isDark ? "text-white" : "text-zinc-900"}`}
                      >
                    ...
                    <p
                      className={`text-sm max-w-md truncate ${isDark ? "text-white/60" : "text-zinc-600"}`}
                    >
                      {profile.bio || "No bio added yet"}
                    </p>
                    <div
                      className={`flex items-center gap-1 text-xs mt-1 ${isDark ? "text-white/40" : "text-zinc-400"}`}
                    >
                      <MapPin size={12} />{" "}
                      {getDisplayLocation(profile.location)
                        .split(",")
                        .slice(-2)
                        .join(", ")
              ...
              <div className="flex flex-wrap gap-2">
                {profile.skills?.slice(0, 3).map((skill: any) => (
                  <span
                    key={skill.id}
                    className={`px-2 lg:px-4 py-1.5 border rounded-lg text-xs ${
                      isDark
              ...
              <div
                className={`text-sm lg:text-base border rounded-lg lg:rounded-2xl flex max-w-sm justify-center capitalize ${isDark ? "bg-white/[0.02] border-white/5" : "bg-[#FDFAF7] border-[#F5EBDA]"}`}
              >
                <StatBox
                  value={
                    ...
                    profile.hourly_rate !== ""
                      ? `$${Math.round(profile.hourly_rate)}`
                      : "-"
                  }
                  sublabel="/Hour"
                  isDark={isDark}
                ...
                />
                <StatBox
                  value={profile.working_distance?.split(" ")[1] || "25"}
                  sublabel="Miles Radius"
                  isDark={isDark}
                />
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                {socialLinks.length > 0 ? (
                  socialLinks.map((link) => {
                    const platformInfo = SOCIAL_ICONS.find(
                      (i) => i.id === link.platform,
                    );
                    return (
                      <a
                        key={link.id}
                  ...
                  <p
                    className={`text-xs italic ${isDark ? "text-white/20" : "text-zinc-400"}`}
                  >
                ...
                <button
                  ...
                  className={`p-2 rounded-lg lg:rounded-xl border hover:bg-[#E8D1AB] hover:text-black transition-all ${
                    isDark
                      ? "bg-white/5 border-white/10 text-white/40"
              ...
              {!mediaPreview ? (
                <>
                  <div className="relative w-32 h-24 mb-6">
                    <div
                      className={`absolute top-0 left-0 w-16 h-16 rounded-lg rotate-[-10deg] ${isDark ? "bg-[#E8D1AB]/10" : "bg-[#E8D1AB]/15"}`}
                    />
                    <div
                      className={`absolute bottom-0 right-0 w-20 h-12 rounded-lg rotate-[5deg] flex items-center justify-center ${isDark ? "bg-[#E8D1AB]/30" : "bg-[#E8D1AB]/40"}`}
                    >
                      <ImageIcon
                        size={20}
                        className={isDark ? "text-[#E8D1AB]" : "text-[#cbb38b]"}
                      />
                    </div>
                  </div>
                  <p
                    className={`text-sm font-bold mb-1 ${isDark ? "text-white" : "text-zinc-800"}`}
                  >
                    Upload Profile Banner
                  </p>
                  <p
                    className={`text-xs max-w-[200px] ${isDark ? "text-white/40" : "text-zinc-500"}`}
                  >
                    Showcase your style with a cover photo or video
                  </p>
                </>
              ) : (
          ...
          {/* NAVIGATION */}
          <div
            className={`flex border-b gap-4 lg:gap-8 overflow-x-auto no-scrollbar ${
          ...
          >
            {tabs.map((tab) => (
              <button
                ...
                className={`pb-2 lg:pb-4 text-sm font-medium transition-all relative whitespace-nowrap ${
                  activeTab === tab
                    ? isDark
                      ? "text-[#E8D1AB]"
                      : "text-[#0A0A0A]"
              ...
              >
                ...
                {activeTab === tab && (
                  <div
                    className={`absolute bottom-0 left-0 w-full h-0.5 ${isDark ? "bg-[#E8D1AB]" : "bg-[#0A0A0A]"}`}
                  />
                )}
              </button>
            ))}
          ...
          <div className="min-h-[400px] pb-10 lg:pb-20">
            {activeTab === "Overview" && (
              <div className="space-y-4 lg:space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                <div
                  className={`border rounded-lg lg:rounded-2xl p-4 lg:p-8 ${
                    isDark
                ...
                >
                  ...
                  {isEditingPersonalInfo ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 lg:gap-y-8 gap-x-20">
                      <InfoField
                        label="First Name"
                        value={capitalizeName(profile.first_name)}
                        placeholder="Add first name"
                        isDark={isDark}
                      />
                      <InfoField
                        label="Last Name"
                        value={capitalizeName(profile.last_name)}
                      ...
                      <InfoField
                        label="Location"
                        value={getDisplayLocation(profile.location)}
                        placeholder="Add location"
                        isDark={isDark}
                      />
                      <InfoField
                        label="Working Distance"
                        value={profile.working_distance}
                        placeholder="Add distance radius"
                        isDark={isDark}
                      />
                    </div>
                  )}
                    ...
                    <div className="flex flex-wrap gap-3">
                      {profile.skills?.length > 0 ? (
                        profile.skills.map((skill: any, index: number) => (
                          <span
                            key={skill.id || index}
                            className={`px-2 lg:px-5 py-2.5 border rounded-lg lg:rounded-xl text-sm ${
                              isDark
                        ...
                        <p
                          className={`italic text-sm ${isDark ? "text-white/20" : "text-black/20"}`}
                        >
                          No skills added yet.
                        </p>
                      )}
                ...
                >
                  <SectionHeader
                    ...
                    onEdit={() => setIsEditingEquipment(!isEditingEquipment)}
                    isDark={isDark}
                  />
                  ...
                  {isEditingEquipment ? (
                    ...
                    <div className="flex flex-wrap gap-3">
                      {equipmentNames.filter(Boolean).length > 0 ? (
                        equipmentNames.filter(Boolean).map((name, index) => (
                          <span
                            key={`${name}-${index}`}
                            className={`px-2 lg:px-5 py-2.5 border rounded-lg lg:rounded-xl text-sm ${
                              isDark
                ...
                >
                  ...
                  {isEditingSecurity ? (
                    <div className="animate-in fade-in zoom-in-95 duration-300">
                      <SecurityForm
                        onSuccess={() => setIsEditingSecurity(false)}
                        isDark={isDark}
                      />
                    </div>
                  ) : (
                    <div
                      className={`flex items-center justify-between p-4 border rounded-lg lg:rounded-2xl ${isDark ? "bg-white/5 border-white/10" : "bg-black/5 border-black/5"}`}
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-10 h-10 rounded-lg flex items-center justify-center ${isDark ? "bg-[#E8D1AB]/10 text-[#E8D1AB]" : "bg-[#E8D1AB]/15 text-[#cbb38b]"}`}
                        >
                          <EyeOff size={20} />
                        </div>
                        <div>
                ...
                {featuredWorks.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div
                      ...
                      className={`border-2 border-dashed rounded-lg lg:rounded-2xl h-[350px] flex flex-col items-center justify-center cursor-pointer transition-all group ${
                        isDark
                    ...
                    {groupedWorks?.map((project: any, pIdx: number) => (
                      <div key={pIdx} className="group flex flex-col">
                        <div
                          className={`h-[350px] rounded-lg lg:rounded-2xl overflow-hidden border relative cursor-pointer ${
                            isDark
                            ...
                            <div className="flex justify-end gap-2">
                              ...
                              <button
                                onClick={(e) => {
                                  ...
                                  setEditingFeaturedWork(
                                    getFeaturedWorkModalItem(project),
                                  );
                ...
                {certifications.length === 0 ? (
                  ...
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                      ...
                      <div
                        onClick={() => certInputRef.current?.click()}
                        className={`border-2 border-dashed rounded-lg lg:rounded-2xl h-[220px] flex flex-col items-center justify-center cursor-pointer transition-all group ${
                          isDark
                            ? "border-white/10 bg-white/[0.02] hover:bg-white/[0.05] hover:border-[#E8D1AB]/40"
                            : "border-black/10 bg-black/[0.01] hover:bg-black/[0.03] hover:border-[#E8D1AB]/60"
                        }`}
                      >
                        ...
                        <p
                          className={`text-sm font-bold mb-1 ${isDark ? "text-white" : "text-black"}`}
                        >
                          Add Certificate
                        </p>
                        <p
                          className={`text-xs text-center px-6 ${isDark ? "text-white/40" : "text-black/40"}`}
                        >
                          Highlight achievements with a professional
                          certificate.
                        </p>
                      </div>
                      ...
                      {certifications.map((cert: any, index: number) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {isPDF ? (
                              <div
                                className={`w-full h-full flex flex-col items-center justify-center ${isDark ? "bg-neutral-900 text-white/20" : "bg-neutral-200 text-black/20"}`}
                              >
                                <FileText size={48} />
                              ...
                              <img
                                ...
                                className="w-full h-full object-cover opacity-80 group-hover:opacity-50 transition-all"
                              />
                            )}
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex justify-center py-4 lg:py-10">
                      <div
                        className={`border rounded-[2.5rem] p-12 w-full max-w-lg relative flex flex-col items-center justify-center text-center shadow-2xl ${isDark ? "bg-[#111111] border-white/5" : "bg-white border-black/5"}`}
                      >
                        ...
                        {/* File Icon Box */}
                        <div
                          className={`w-16 h-20 border rounded-2xl flex items-center justify-center mb-6 shadow-xl ${
                            isDark
                        ...
                        {/* File Details */}
                        <h3
                          className={`text-xl font-bold mb-1 ${isDark ? "text-white" : "text-black"}`}
                        >
                          My Resume
                        </h3>
                        <p
                          className={`text-sm mb-10 font-medium ${isDark ? "text-white/40" : "text-black/40"}`}
                        >
                          Uploaded on{" "}
                          {new Date(
                            resumeFile.created_at || Date.now(),
                          ).toLocaleDateString()}
                        </p>
                        ...
                        <div className="flex items-center gap-4 w-full">
                          <button
                            onClick={() =>
                              window.open(
                                `${S3_BASE_URL}${resumeFile.file_path}`,
                                "_blank",
                              )
                            }
                            className={`text-sm lg:text-base w-full font-bold px-5 lg:px-10 py-2.5 lg:py-3.5 rounded-lg lg:rounded-2xl transition-all active:scale-95 shadow-lg ${
                              isDark
                          ...
                          <button
                            onClick={() => resumeInputRef.current?.click()}
                            className={`text-sm lg:text-base w-full font-bold px-5 lg:px-10 py-2.5 lg:py-3.5 rounded-lg lg:rounded-2xl transition-all active:scale-95 border ${
                              isDark
                  ...
                  return (
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div
                          onClick={() => {
                            const mappedLinks = portfolioLinks.map(
                              (l: any) => ({
                                id: l.crew_files_id,
                                url: l.file_path,
                                platform: l.tag,
                                name:
                                  PORTFOLIO_ICONS.find((p) => p.id === l.tag)
                                    ?.label || l.tag,
                              }),
                            );
                          ...
                          className={`border-2 border-dashed rounded-lg lg:rounded-2xl h-[220px] lg:h-auto min-h-[220px] flex flex-col items-center justify-center cursor-pointer transition-all group ${
                            isDark
                              ? "border-white/10 bg-white/[0.02] hover:bg-white/[0.05] hover:border-[#E8D1AB]/40"
                              : "border-black/10 bg-black/[0.01] hover:bg-black/[0.03] hover:border-[#E8D1AB]/60"
                          }`}
                        >
                          ...
                          <p
                            className={`text-xs text-center px-6 ${isDark ? "text-white/40" : "text-black/40"}`}
                          >
                            Share your external work links here.
                          </p>
                        </div>

                        {portfolioLinks.map((link: any, index: number) => {
                          const platform = PORTFOLIO_ICONS.find(
                            (p) => p.id === link.tag,
                          );
                          return (
                            <div
                              key={link.crew_files_id || index}
                              className={`border rounded-2xl p-4 flex flex-col gap-4 group transition-all shadow-xl ${
                                isDark
                            ...
                            >
                              ...
                              <div className="space-y-1">
                                <p
                                  className={`text-sm font-bold uppercase tracking-wider ${isDark ? "text-white" : "text-black"}`}
                                >
                                  {platform?.label || "Portfolio Link"}
                                </p>
                                <p
                                  className={`text-xs truncate ${isDark ? "text-white/40" : "text-black/40"}`}
                                >
                                  {link.file_path}
                                </p>
                              </div>
                              ...
                              <button
                                ...
                                className={`w-full border py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 group/btn ${
                                  isDark
        ...
        {lightboxData.isOpen && lightboxData.project && (
          <div
            className={`fixed inset-0 z-[100] backdrop-blur-xl flex flex-col animate-in fade-in duration-300 ${isDark ? "bg-black/95" : "bg-white/95"}`}
          >
            {/* Top Bar */}
            <div className="flex items-start justify-between gap-6 p-6 lg:p-8">
              <div className="flex flex-col gap-3">
                ...
                <p
                  className={`text-xs uppercase tracking-[0.22em] ${isDark ? "text-white/40" : "text-black/40"}`}
                >
                  Media {lightboxData.index + 1} of{" "}
                  {lightboxData.project.images.length}
                </p>
              </div>
              <div className="flex items-center gap-4">
                ...
                <button
                  onClick={() =>
                    confirmDelete(
                      "file",
                      lightboxData.project.images[lightboxData.index],
                    )
                  }
            ...
            <div className="flex-1 relative flex items-center justify-center px-4 py-6 lg:px-20 lg:py-8">
              ...
              <div className="w-full max-w-5xl">
                <div
                  className={`relative mx-auto flex aspect-[4/3] max-h-[calc(100vh-16rem)] w-full items-center justify-center overflow-hidden rounded-2xl shadow-2xl ${isDark ? "bg-[#050505]" : "bg-neutral-950"}`}
                >
                  <img
                ...
                <button
                  ...
                  className={`w-20 h-14 rounded-lg overflow-hidden border-2 transition-all shrink-0 ${
                    lightboxData.index === idx
                      ? isDark
                        ? "border-[#E8D1AB] scale-110"
                        : "border-[#cbb38b] scale-110"
                      : "border-transparent opacity-40 hover:opacity-100"
                  }`}
                >
                  <img
                    src={`${S3_BASE_URL}${img.file_path}`}
                    alt=""
                    className="w-full h-full object-cover"
                  />
                </button>
            ...
            <div className="relative w-full max-w-5xl h-full flex flex-col">
              ...
              <div className="flex-1 bg-white rounded-2xl overflow-hidden relative shadow-2xl">
                ...
                <button
                  onClick={() =>
                    window.open(
                      `${S3_BASE_URL}${previewCert.file_path}`,
                      "_blank",
                    )
                  }
        ...
        {playingVideo && (
          <div
            className={`fixed inset-0 z-[120] backdrop-blur-2xl overflow-y-auto animate-in fade-in duration-500 ${isDark ? "bg-black/98" : "bg-white/98"}`}
          >
            {/* Top Bar - Sticky so the close button is always visible even when scrolling */}
            <div
              className={`sticky top-0 z-50 flex items-center justify-between p-4 lg:p-10 bg-gradient-to-b to-transparent pointer-events-none ${isDark ? "from-black/95 via-black/80" : "from-white/95 via-white/80"}`}
            >
              <div className="space-y-1 pointer-events-auto">
                <h3
                  className={`text-xs lg:text-sm font-black uppercase tracking-[0.3em] ${isDark ? "text-white" : "text-black"}`}
                >
                  Portfolio Player
                </h3>
                <div className="flex items-center gap-2">
                  <span
                    className={`w-1.5 h-1.5 rounded-full animate-pulse ${isDark ? "bg-[#E8D1AB]" : "bg-[#cbb38b]"}`}
                  />
                  <p
                    className={`text-xs uppercase font-bold tracking-widest ${isDark ? "text-white/30" : "text-black/40"}`}
                  >
                    Now Playing
                  </p>
                </div>
              ...
              <button
                ...
                className={`p-3 lg:p-4 border rounded-full transition-all active:scale-90 shadow-lg pointer-events-auto ${
                  isDark
            ...
            <div className="w-full max-w-6xl mx-auto px-4 pb-24 pt-2 lg:pt-10">
              <div
                className={`w-full aspect-video bg-black rounded-xl lg:rounded-[2rem] overflow-hidden border relative ${isDark ? "shadow-[0_0_100px_rgba(0,0,0,0.8)] border-white/10" : "shadow-2xl border-black/10"}`}
              >
                <iframe
        ...
        {isPageLoading && (
          <div
            className={`fixed inset-0 z-[9999] backdrop-blur-sm flex items-center justify-center ${isDark ? "bg-black/80" : "bg-white/80"}`}
          >
            <div className="flex flex-col items-center gap-4">
              <Loader2 className="h-10 w-10 animate-spin text-[#E8D1AB]" />
              <p
                className={`text-sm tracking-wide ${isDark ? "text-white/80" : "text-black/80"}`}
              ></p>
            </div>
          </div>
        )}
