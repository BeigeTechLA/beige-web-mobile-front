...
function StripePaymentFormMulti({
  ...
  const [serviceAgreementCheckedInModal, setServiceAgreementCheckedInModal] =
    useState(true);
  const [expandedAgreementSection, setExpandedAgreementSection] = useState("01");
      ...
      <div className="flex gap-3 bg-[#2A2A2A] rounded-[10px] p-2 lg:p-4 items-center mt-2">
        ...
        <p className="text-sm text-[#999]">
          ...
          <button
            type="button"
            onClick={() => {
              setServiceAgreementCheckedInModal(acceptServiceAgreement);
        ...
        <div className="fixed inset-0 z-[100] bg-black/70 p-3 lg:p-6 flex items-center justify-center">
          <div className="w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-2xl border border-white/10 bg-black">
            <div className="flex items-center justify-between px-6 lg:px-7 py-5 lg:py-6 border-b border-white/10">
              ...
              <button
                type="button"
                onClick={() => setIsServiceAgreementOpen(false)}
            ...
            <div className="p-4 lg:p-5 overflow-y-auto max-h-[calc(90vh-90px)]">
              ...
              <div className="rounded-lg border border-white/10 overflow-hidden">
                {SERVICE_AGREEMENT_SECTIONS.map((section) => {
                  const isOpen = expandedAgreementSection === section.id;
                  return (
                    <div key={section.id} className="border-b border-white/10 last:border-b-0">
                      <button
                        type="button"
                        onClick={() =>
                          setExpandedAgreementSection(isOpen ? "" : section.id)
                        }
              ...
              <label className="mt-4 rounded-md bg-[#171717] px-3 py-2 flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={serviceAgreementCheckedInModal}
                  onChange={(e) =>
                    setServiceAgreementCheckedInModal(e.target.checked)
                  }
                />
              ...
              <div className="mt-4">
                <Button
                  type="button"
                  onClick={() => {
                    if (!serviceAgreementCheckedInModal) return;
                    setAcceptServiceAgreement(true);
                    setIsServiceAgreementOpen(false);
                  }}
                  disabled={!serviceAgreementCheckedInModal}
                  className="h-11 bg-[#E8D1AB] hover:bg-[#dcb98a] text-black font-medium rounded-[10px] disabled:opacity-50"
                >
                  Accept & Continue
                </Button>
              </div>
