...
interface EditBookingDetailsFormProps {
...
export default function EditBookingDetailsForm({ leadId, initialBookingData, onSuccess, onCancel, isModal, hideCreativeSelector, projectId, isDark = true }: EditBookingDetailsFormProps) {
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <DatePicker
                ...
                isDark={isDark}
              />
              <DropdownSelect title="Start Time" options={timeOptions} value={getStartTimeKey()} onChange={handleStartTimeChange} bgColour={isDark ? "bg-[#101010]" : "bg-[#F4F5F7]"}
                isDark={isDark} />
              <DropdownSelect title="End Time" options={filteredEndTimeOptions} value={getEndTimeKey()} onChange={handleEndTimeChange} bgColour={isDark ? "bg-[#101010]" : "bg-[#F4F5F7]"}
                isDark={isDark} />
            </div>
          </>
        ) : (
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className={`absolute right-0 top-14 z-50 border p-5 rounded-2xl shadow-2xl w-[320px] ${isDark ? "bg-[#111] border-white/10" : "bg-white border-gray-200"
                    ...
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => setIsCalendarOpen(false)} className={`rounded-full p-1 transition-colors ${isDark ? "hover:bg-white/10 text-white" : "hover:bg-black/5 text-black"}`}>
                          <X size={18} />
                        </button>
                      </div>
                    </div>
                ...
                <div className="flex gap-4">
                  ...
                  >
                    ...
                    <div className={`w-6 h-6 lg:w-8 lg:h-8 rounded-full flex items-center justify-center ${sameTimingsMulti ? "bg-black" : (isDark ? "border border-[#E5E5E5]" : "border border-[#0000004D]")}`}>
                      {sameTimingsMulti && <div className="w-2 h-2 rounded-full bg-[#E8D1AB] m-auto mt-1.5" />}
                    </div>
                  </button>
                  <button
                    onClick={() => {
                ...
                {sameTimingsMulti ? (
                  <div className="animate-in slide-in-from-top-4 duration-300">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <DropdownSelect
                        title="End Time"
                        options={filteredEndTimeOptions} value={getEndTimeKey()} onChange={handleEndTimeChange}
                        bgColour={isDark ? "bg-[#101010]" : "bg-[#F4F5F7]"}
                        isDark={isDark}
                      />
                    </div>
                        ...
                        <div key={date.toISOString()} className={`border rounded-2xl {isExpanded ? "overflow-visible" : "overflow-hidden"} ${isDark ? "border-white/10 bg-[#171717]" : "border-black/10 bg-white shadow-sm"} ${isExpanded ? "overflow-visible" : "overflow-hidden"}`}>
                          ...
                          <AnimatePresence>
                            {isExpanded && (
                              ...
                              >
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                  <DropdownSelect title="Start Time" options={timeOptions} value={multiDayTimes[dateKey]?.startKey || ""} onChange={(v) => handleMultiDayStartTimeChange(dateKey, v)} bgColour={isDark ? "bg-[#101010]" : "bg-[#F4F5F7]"}
                                    isDark={isDark} />
                                  <DropdownSelect title="End Time" options={filteredEndTimeOptions} value={multiDayTimes[dateKey]?.endKey || ""} onChange={(v) => handleMultiDayEndTimeChange(dateKey, v)} bgColour={isDark ? "bg-[#101010]" : "bg-[#F4F5F7]"}
                                    isDark={isDark} />
                                </div>
        ...
        {formData.editsNeeded && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8 animate-in slide-in-from-top-4 duration-300">
            {formData.contentType.includes("videographer") && videoEditTypeOptions.length > 0 && (
              <MultiSelectDropdown title="Video Edit Type" options={videoEditTypeOptions} value={formData.videoEditTypes} onChange={(v) => updateData({ videoEditTypes: v })} bgColour={isDark ? "bg-[#101010]" : "bg-[#F4F5F7]"}
                isDark={isDark} />
            )}
            {formData.contentType.includes("photographer") && photoEditTypeOptions.length > 0 && (
              <div>
                <MultiSelectDropdown title="Photo Edit Type" options={photoEditTypeOptions} value={formData.photoEditTypes} onChange={(v) => updateData({ photoEditTypes: v })} bgColour={isDark ? "bg-[#101010]" : "bg-[#F4F5F7]"}
                isDark={isDark} />
