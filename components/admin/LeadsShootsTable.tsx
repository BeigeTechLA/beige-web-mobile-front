...
import { ChevronDown, ChevronUp, Loader2, Calendar as CalendarIcon, Search, ChevronLeft, ChevronRight } from "lucide-react";
            ...
            <div className={`transition-opacity duration-200 ${isFetching ? 'opacity-50' : 'opacity-100'}`}>
              {currentLeads.map((lead) => (
                <div
                  ...
                  className={`cursor-pointer px-4 pb-4 transition-colors ${isDark ? "border-white/5 hover:bg-white/[0.02]" : "border-[#E5E5E5] hover:bg-black/[0.02]"}`}
                >
                  <div className="flex items-center justify-between gap-1 ">
                    <div className="min-w-0 flex items-center gap-3">
                      ...
                      <div className="shrink-0 w-10 h-10 rounded-lg bg-[#FFF6D9] flex items-center justify-center text-black font-medium text-sm">
                        {lead.customerName.split(" ").map((n) => n[0]).join("")}
                      </div>
                      <div className="min-w-0 ">
                        <p className={`truncate text-sm font-medium ${isDark ? "text-white" : "text-[#000000]"}`}>{lead.customerName}</p>
                      </div>
                    </div>
        ...
        <div className={`flex justify-between items-center p-4 border-t transition-colors ${isDark ? "bg-[#101010] border-white/5" : "bg-[#FFFCF6] border-[#E5E5E5]"}`}>
          ...
          <div className="flex gap-2 items-center justify-center sm:justify-end w-full sm:w-auto max-w-full min-w-0 overflow-hidden">
            <button
              ...
              className={`px-1.5 lg:px-3 py-1.5 text-xs font-medium rounded-lg border transition-all disabled:opacity-30 disabled:cursor-not-allowed ${isDark ? "bg-[#1A1A1A] text-white/60 border-white/5 hover:bg-white/10" : "bg-white text-[#333] border-[#E5E5E5] hover:bg-zinc-100"}`}
            >
