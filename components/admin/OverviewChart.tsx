...
export default function OverviewChart({ externalSelectedDate }: OverviewChartProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // CRITICAL FIX: Determine the "effective" range immediately
        // Don't rely on the 'range' state variable here because it might be stale
    ...
    fetchData();
  }, [range, externalSelectedDate]); // Both dependencies are correct now

  const getGrowthLabel = () => {
    // Determine growth label based on effective range logic
    ...
    <div className="bg-[#171717] border border-[#3D3D3D] rounded-2xl p-5 w-full text-white mt-5 lg:mt-9">
      ...
      <div className="flex justify-between items-center mb-5 lg:mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <SelectTrigger className="w-[130px] bg-zinc-900 border-[#3D3D3D] rounded-full h-9 text-[10px] lg:text-xs text-zinc-400 focus:ring-0">
              <SelectValue placeholder="Range" />
            </SelectTrigger>
            <SelectContent className="bg-[#111111] border-[#3D3D3D] text-white">
      ...
      {/* Metric Cards Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-10 bg-[#101010] rounded-2xl p-4">
        {metrics.map((m: any) => {
          ...
          return (
            <div
              ...
              className={`relative group cursor-pointer rounded-lg p-4 border border-transparent transition-all duration-200 ${
                isActive ? 'bg-[#ECD7B4] text-[#171717]' : 'bg-[#101010] text-white hover:border-white/30'
              }`}
            >
              {/* Tooltip for CPs */}
              {m.id === 'cps' && m.details && (
                <div className="absolute -top-36 left-1/2 -translate-x-1/2 w-40 bg-[#1A1A1A] border border-[#3D3D3D] rounded-xl p-3 shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 pointer-events-none mb-2">
                  <div className="space-y-2">
                    <p className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold border-b border-[#3D3D3D] pb-1">CP Breakdown</p>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-[#0DAE3D]">Approved</span>
                    ...
                    <div className="pt-1 border-t border-[#3D3D3D] flex justify-between items-center">
              ...
              <div className="flex justify-between items-start mb-6">
                <span className={`text-xs font-medium ${isActive ? 'text-black/70' : 'text-zinc-400'}`}>
                  {m.label}
                </span>
                <div className={`p-1.5 rounded-full ${isActive ? 'bg-[#171717] text-[#E8D1AB]' : 'bg-[#2C2C2C] text-white/60'}`}>
                  <m.icon size={16} />
                </div>
              </div>
              
              <div className="text-2xl font-bold mb-2">
                {isLoading ? <div className="h-8 w-12 bg-white/10 animate-pulse rounded" /> : m.value}
              </div>

              <div className={`text-[10px] flex gap-1 items-center ${isActive ? 'text-[#171717]' : 'text-white/70'}`}>
              ...
              {m.id === 'cps' && (
                <button
                  ...
                  className={`absolute bottom-3 right-3 p-1 rounded-full transition-colors ${isActive ? 'text-black/70 hover:bg-black/10' : 'text-zinc-400 hover:bg-white/10'}`}
                >
      ...
      <div className="h-[310px] lg:h-[350px] w-full relative">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData}>
            ...
            />
            <Tooltip
              contentStyle={{ backgroundColor: '#1A1A1A', borderRadius: '8px', border: '1px solid #3D3D3D', color: '#fff' }}
              itemStyle={{ color: '#E5D5B8' }}
