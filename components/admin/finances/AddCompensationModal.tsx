...
const getCompensationBreakdown = (
  ...
  const budgetLimit = Math.max(getCreatorTargetAmount(form, rateType), 0);
  ...
  if (compensationMethod === "manual") {
    const base = roundMoney(Math.max(parseAmount(form.baseTarget || form.base || "0"), 0));
    const editing = roundMoney(Math.max(editingRaw, 0));
    const travel = roundMoney(Math.max(travelRaw, 0));
    const bonus = roundMoney(Math.max(bonusRaw, 0));
...
export default function AddCompensationModal({
  ...
  const syncCreatorFormsForCompensationMethod = useCallback(() => {
    ...
    setCreatorForms((prev) => currentShoot.creators.reduce<Record<string, CreatorFormState>>((acc, creator) => {
      ...
      }, creatorRateType, compensationMethod).base;
  ...
  const getCreatorBreakdownForForm = (form: Partial<CreatorFormState>, rateType: "flat" | "hourly") =>
    getCompensationBreakdown(form, rateType, compensationMethod);
    ...
    setCreatorForms((prev) => ({
      ...
      [creatorId]: {
        ...(() => {
          ...
          const budgetLimit = getCreatorTargetAmount(draft, nextRateType);
          ...
          if (compensationMethod === "manual") {
            const manualBaseAmount = roundMoney(Math.max(parseAmount(field === "baseTarget" ? nextValue : existing.baseTarget || existing.base || "0"), 0));
            const nextBase = field === "baseTarget"
              ? getAmountDisplayValue(nextValue, manualBaseAmount)
            ...
            return {
              ...
              baseTarget: field === "baseTarget" ? nextBase : (existing.baseTarget || "0"),
              editing: field === "editing" ? getAmountDisplayValue(nextValue, roundMoney(Math.max(editingCandidate, 0))) : String(roundMoney(Math.max(editingCandidate, 0))),
              travel: field === "travel" ? getAmountDisplayValue(nextValue, roundMoney(Math.max(travelCandidate, 0))) : String(roundMoney(Math.max(travelCandidate, 0))),
              bonus: field === "bonus" ? getAmountDisplayValue(nextValue, roundMoney(Math.max(bonusCandidate, 0))) : String(roundMoney(Math.max(bonusCandidate, 0))),
              hourlyRate:
                isManualHourly
                  ? (field === "hourlyRate"
                      ? nextValue
          ...
          const nextBaseTarget = nextRateType === "hourly"
  ...
  const handleAdvanceSubmit = (payload: { reason: string; advanceAmount: string; paymentDate: Date | null; proofFile?: File | null }) => {
    ...
    setCreatorForms((prev) => ({
      ...
      [advanceCreatorId]: {
        ...
        advanceProofFile: enableAdvanceProofUpload ? payload.proofFile || null : null,
      },
    }));
    setAdvanceCreatorId(null);
  };
  ...
  const handleFormSubmit = async () => {
    ...
    const creators = await Promise.all(selectedCreators.map(async (creatorId) => {
      ...
      const uploadedProof = enableAdvanceProofUpload && advanceAmount > 0 && form.advanceProofFile
        ? await cpCompensationApi.uploadPaymentProof(form.advanceProofFile, {
        ...
        ...(advanceAmount > 0
          ? {
              advance: {
                ...
                proof_url: uploadedProof?.proof_url || uploadedProof?.file_path || undefined,
                proof_file_path: uploadedProof?.file_path || undefined,
                proof_file_name: form.advanceProofFile?.name || undefined,
                payment_mode: uploadedProof ? "advance_payment" : undefined,
                transaction_reference: uploadedProof ? `ADV-${creatorId}-${Date.now()}` : undefined,
              },
            }
          : {}),
      };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-[#101010CC] font-sans backdrop-blur-sm animate-in fade-in duration-200 p-4 lg:p-0">
                          ...
                          {form.rateType === "hourly" ? (
                            <div className={`grid grid-cols-1 gap-3 lg:grid-cols-[minmax(160px,1fr)_auto_auto_auto_auto] items-center rounded-xl border px-3 py-3 mt-2 ${isDark ? "border-[#5A5A5F] bg-[#0C0C0C]/40" : "border-[#e5e5e5] bg-[#D7D7D7]/40"}`}>
                              ...
                              <p className={`lg:col-span-4 text-[10px] lg:text-[11px] ${isDark ? "text-white/40" : "text-black/40"}`}>
                                {compensationMethod === "manual"
                                  ? "Hourly base stays separate. Editing, travel, and bonus do not reduce it."
                            ...
                            <div className={`relative rounded-xl border px-4 py-2 mt-2 ${isDark ? "border-[#5A5A5F] bg-[#0C0C0C]/40" : "border-[#e5e5e5] bg-[#D7D7D7]/40"}`}>
                              ...
                              <p className={`mt-1 text-[11px] lg:text-xs ${isDark ? "text-white/40" : "text-black/40"}`}>
                                {compensationMethod === "manual"
                                  ? "Manual amount. Editing, travel, and bonus stay separate."
      ...
      <AdvancePaymentModal
        ...
        showProofUpload={enableAdvanceProofUpload}
        requireProofUpload={enableAdvanceProofUpload}
