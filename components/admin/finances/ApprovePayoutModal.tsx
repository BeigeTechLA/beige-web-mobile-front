...
export type ApprovePayoutModalProps = {
  isOpen: boolean;
  onClose: () => void;
      ...
      <div className={`relative max-h-[90vh] w-full lg:max-w-xl overflow-y-auto rounded-[16px] border transition-colors duration-200 ${isDark
        ...
        <div className="space-y-3 lg:space-y-7 p-4 lg:p-7">
          <div className={`rounded-xl border px-4 py-3.5 lg:px-5 lg:py-4 border-[#E8D1AB33] bg-[#E8D1AB33]`}>
            <p className={`text-xs lg:text-sm ${isDark ? "text-white" : "text-black"}`}>
              Approve this compensation payout? This action will lock the payout amount and mark it ready for payment processing.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className={`text-sm lg:text-base font-medium flex flex-col lg:flex-row gap-2 justify-between p-3 lg:p-4 rounded-lg border ${isDark ? "bg-[#171717] border-[#3D3D3D]" : "bg-[#F2F3F5] border-[#8E8E8E]"}`}>
              ...
              <div className={`flex items-center justify-center lg:justify-start rounded-full border-[1px] px-3 py-2 lg:px-5 lg:py-3 text-[#E8D1AB] text-sm lg:text-base ${isDark ? "bg-black border-white/20" : "bg-white border-black/20"}`}>
        ...
        <div className={`sticky bottom-0 inset-x-0 p-4 lg:p-6 grid grid-cols-2 gap-4 z-40 mt-auto ${isDark ? "bg-black" : "bg-white"}`}>
          ...
          <Button
            ...
            onClick={() => {
              console.log("Approved Submission Fired");
