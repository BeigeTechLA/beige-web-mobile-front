...
export type PayoutMetricCard = {
  id: string;
  label: string;
  value: React.ReactNode;
  helperText: string;
  icon: React.ComponentType<{ size?: number }> | React.ElementType;
};

interface PayoutMetricCardsProps {
  metrics: PayoutMetricCard[];
  activeId?: string;
...
export default function PayoutMetricCards({
  ...
  return (
    <section
      ...
      <div className="flex justify-between items-center mb-5 gap-4">
        ...
        {onRangeChange && (
          <Select value={rangeValue} onValueChange={onRangeChange}>
            <SelectTrigger
              className={`w-[110px] rounded-full h-9 text-[10px] lg:text-xs focus:ring-0 ${isDark
      ...
      <div
        className={`grid grid-cols-1 lg:grid-cols-4 gap-4 rounded-2xl p-4 transition-colors duration-300 ${isDark ? "bg-[#101010]" : "bg-white border border-[#F0F0F0]"}`}
      >
        {metrics.map((metric) => {
          ...
          return (
            <div
              ...
              className={`relative rounded-lg p-4 border transition-all duration-200 ${isActive
            ...
            >
              ...
              <div className="text-xl lg:text-[26px] leading-normal font-semibold mb-2 lg:mb-3">
                {metric.value}
              </div>
              <div
