...
const mapTransactionItem = (item: FinanceTransactionItem, index: number): TransactionRow => {
  ...
  return {
    id: String(item.finance_transaction_id ?? item.id ?? transactionIdValue),
    transactionId: String(transactionIdValue),
    shootId: String(bookingId).startsWith("#") ? String(bookingId) : `#${bookingId}`,
    clientName,
    date: formatDate(item.transaction_date || item.created_at),
...
export default function TransactionIdTable({ selectedDate }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const response = await adminApi.getFinanceTransactions(date_on ? { date_on } : {});
        if (response?.success === false) throw new Error(response?.error || "Failed to fetch transactions");
        const items = getArrayFromPayload(response);
        setRows(items.map((item, index) => mapTransactionItem(item as FinanceTransactionItem, index)));
      } catch (error) {
      ...
      <div className={`flex flex-col gap-4 p-5 lg:p-6 border-b ${isDark ? "border-[#333333]" : "border-[#E5E5E5]"}`}>
        <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className={isDark ? "text-white text-[18px]" : "text-[#323232] text-[18px]"}>Transaction ID</h3>
          </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {loading ? (
              ...
              visibleRows.map((row) => (
                <tr key={row.id} className={`border-b ${isDark ? "border-[#222222] hover:bg-white/[0.02]" : "border-[#F5F5F5] hover:bg-zinc-50"}`}>
                  <td className={`py-6 px-6 whitespace-nowrap text-[18px] ${isDark ? "text-[#E0E0E0]" : "text-[#333]"}`}>{row.transactionId}</td>
                  ...
                  <td className="py-6 px-6"><TransactionStatusBadge status={row.status} /></td>
