...
const CREATOR_DATA = [
  { id: 1, name: "Raj Verma", type: "Videographer", shootId: "#12345", shootDate: "01/06/2026", customer: "Alex Morgan", amount: "$50,000", payout: "$12,500", margin: "18.5%", status: "Pending", avatar: { initials: "RV", color: "bg-[#F3E5F5] text-[#9C27B0]" } },
  { id: 2, name: "Daniel Foster", type: "Videographer + Photographer", shootId: "#12345", shootDate: "01/06/2026", customer: "Ethan Carter", amount: "$20,000", payout: "$10,000", margin: "16.2%", status: "Partially Paid", image: "https://api.dicebear.com/7.x/avataaars/svg?seed=Daniel" },
  { id: 3, name: "Liam Parker", type: "Videographer", shootId: "#12345", shootDate: "01/06/2026", customer: "Maya Ross", amount: "$45,000", payout: "$25,000", margin: "8.5%", status: "Finance Approval", avatar: { initials: "LP", color: "bg-[#E8F5E9] text-[#4CAF50]" } },
  { id: 4, name: "Noah Bennett", type: "Photographer", shootId: "#12345", shootDate: "01/06/2026", customer: "John Lee", amount: "$10,000", payout: "$5,000", margin: "18.5%", status: "Approved", image: "https://api.dicebear.com/7.x/avataaars/svg?seed=Noah" },
  { id: 5, name: "James Carter", type: "Videographer + Photographer", shootId: "#12345", shootDate: "01/06/2026", customer: "Raj Yadhav", amount: "$1,00,000", payout: "$64,000", margin: "18.5%", status: "Fully Paid", image: "https://api.dicebear.com/7.x/avataaars/svg?seed=James" },
  { id: 6, name: "Lucas Reed", type: "Videographer + Photographer", shootId: "#12345", shootDate: "01/06/2026", customer: "Daniel Roberts", amount: "$2,00,000", payout: "$1,10,000", margin: "18.5%", status: "Pending", avatar: { initials: "LR", color: "bg-white text-black" } },
];
...
export default function CreatorsCompensationTable() {
  ...
  const filteredRows = useMemo(() => {
    return CREATOR_DATA.filter((row) => {
    ...
    <section className={`w-full rounded-2xl border overflow-hidden ${isDark ? "bg-[#111111] border-[#333333]" : "bg-white border-[#E5E5E5]"}`}>
      <div className={`p-5 lg:p-6 border-b ${isDark ? "border-[#333333]" : "border-[#E5E5E5]"}`}>
        ...
        <div className="relative w-full">
          ...
          <input
            type="text"
            placeholder="Search by Creator Name, Customer..."
            value={searchQuery}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className={`text-sm font-medium border-b ${isDark ? "text-[#E8D1AB] border-[#333333]" : "text-[#777] border-[#E5E5E5] bg-[#FFFCF6]"}`}>
              <th className="py-5 px-6 font-medium whitespace-nowrap">Creator Name</th>
              <th className="py-5 px-6 font-medium whitespace-nowrap">Shoot ID</th>
              <th className="py-5 px-6 font-medium whitespace-nowrap">Customer</th>
              <th className="py-5 px-6 font-medium whitespace-nowrap">Shoot Amount</th>
                ...
                >
                  <td className="py-6 px-6">
                    <div className="flex items-center gap-3">
                      {row.image ? (
                        <img src={row.image} alt={row.name} className="w-12 h-12 rounded-lg object-cover bg-white/5" />
                      ) : (
                        <div className={`w-12 h-12 rounded-lg ${row.avatar?.color || "bg-white text-black"} flex items-center justify-center text-sm font-bold`}>
                          {row.avatar?.initials}
                        </div>
                      )}
                  ...
                  <td className="py-6 px-6">
                    <div className="flex flex-col">
                      <span className={`text-[16px] font-semibold ${isDark ? "text-[#E0E0E0]" : "text-[#333]"}`}>{row.shootId}</span>
                      <span className={`text-xs ${isDark ? "text-white/40" : "text-[#777]"} mt-0.5`}>Created on {row.shootDate}</span>
                    </div>
                  </td>
                  <td className="py-6 px-6">
                    <span className={`text-[16px] font-semibold ${isDark ? "text-white" : "text-[#171717]"}`}>{row.customer}</span>
                  </td>
                  <td className={`py-6 px-6 text-[16px] ${isDark ? "text-[#E0E0E0]" : "text-[#333]"}`}>{row.amount}</td>
