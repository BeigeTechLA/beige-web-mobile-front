...
interface MobileShootRowProps {
  shoot: ShootRecord;
  openCardActionId: string | null;
  setOpenCardActionId: React.Dispatch<React.SetStateAction<string | null>>;
}
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={`px-3 py-1.5 rounded-full text-xs font-semibold leading-none whitespace-nowrap inline-block ${style}`}>
      {status}
    </span>
  );
};

export const MobileShootRow = ({
  shoot,
  openCardActionId,
  setOpenCardActionId,
}: MobileShootRowProps) => {
  ...
  return (
    <div className={`w-full overflow-visible transition-all duration-300 ${isMissingInfo
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          {isMissingInfo && (
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold whitespace-nowrap ${isDark ? "bg-red-500/20 text-red-400 border border-red-500/30" : "bg-red-100 text-red-600 border border-red-200"}`}>
              MISSING INFO
            </span>
          )}
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className={`w-full transition-colors duration-300 ${isDark ? "bg-[#202020]" : "border-[#E5E5E5] bg-[#F9F9F9]"}`}
          >
            <div className="pt-0 pr-5 pb-7 pl-14 space-y-4">
              <div className="grid grid-cols-2 gap-y-3 w-full">
                ...
                {/* Fixed Action Dropdown Cell Block */}
                <div className="relative text-right overflow-visible" data-card-actions="true">
