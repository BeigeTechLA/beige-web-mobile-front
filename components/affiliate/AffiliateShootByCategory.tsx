...
export default function AffiliateShootByCategory({ externalSelectedDate }: { externalSelectedDate?: Date | null }) {
      ...
      try {
        ...
        if (!response.error && response.data) {
          ...
          const mappedData = rawCategories.map((item: any, index: number) => ({
            name: item.label || item.category_name || item.name || 'Unknown',
  ...
  return (
    <div className={`border rounded-2xl w-full max-w-md h-full flex flex-col transition-colors duration-300 ${
      isDark ? "bg-[#171717] border-[#3D3D3D] text-white" : "bg-[#FFF] border-[#E5E5E5] text-[#323232]"
    }`}>
        ...
        <div className={`flex px-2 border-b rounded-b-2xl ${isDark ? "border-b-[#3D3D3D]" : "bg-[#FFFCF6] border-b-[#E5E5E5]"}`}>
          {TABS.map((tab) => (
            <button
              ...
              className={`flex-1 pt-5 pb-3.5 px-4 text-sm font-medium transition-all border-b-[3px] ${
                activeTab === tab
          ...
          <>
            ...
            <div className="relative h-64 w-full mb-10 shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-3xl font-bold">{totalCount.toString().padStart(2, '0')}</span>
                <span className={`text-lg capitalize tracking-wider ${isDark ? "text-white/80" : "text-[#323232CC]"}`}>Total Shoots</span>
              </div>
            </div>
