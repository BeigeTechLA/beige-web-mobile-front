...
import { adminApi } from "@/lib/api"; /* --- Replace when create client side API --- */
...
export default function NotesDrawer({
  ...
  const fetchNotes = async ({ silent = false }: { silent?: boolean } = {}) => {
    ...
    const response = await adminApi.getShootNotes(bookingId); /* --- Replace when create client side API --- */
    if (!response?.success) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = replyingToId
        ? await adminApi.replyToShootNote(bookingId, replyingToId, { note: payloadNote, attachments: selectedAttachments }) /* --- Replace when create client side API --- */
        : await adminApi.addShootNote(bookingId, { note: payloadNote, attachments: selectedAttachments });  /* --- Replace when create client side API --- */
  ...
  const handleReaction = async (messageId: string, emoji: string) => {
    ...
    try {
      const response = await adminApi.reactToShootNote(bookingId, messageId, { reaction });   /* --- Replace when create client side API --- */
      if (!response?.success) {
  ...
  const handleDeleteNote = async (noteId: number) => {
    ...
    try {
      const response = await adminApi.deleteShootNote(bookingId, noteId);  /* --- Replace when create client side API --- */
      if (!response?.success) {
