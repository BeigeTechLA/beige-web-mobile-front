...
export const Step1ProjectDetails = ({ data, updateData, onNext }: Props) => {
  ...
  const handleNext = () => {
    ...
    // Content Type is only required if we are actually shooting
    if (data.serviceType !== "edit_files" && (!data.contentType || data.contentType.length === 0)) {
      toast.error("Selection Required", { description: "Please select at least one content type." });
      return;
    }

    // Shoot Type is only required if we are actually shooting (We cannot select the edit type without selecting shoot type. Hence this field is mandatory)
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <DropdownSelect
              title="Shoot Type"
              options={shootTypes}
              value={shootType || data.shootType}
              onChange={(value) => {
                setShootType(value);
                updateData({
                  shootType: value,
            ...
            <MultiSelectDropdown
              ...
              bgColour={"bg-[#101010]"}
              isDisabled={data.shootType === ""} //Enable only if shoot type is selected
            />
          </div>
        </div>
      </div>
