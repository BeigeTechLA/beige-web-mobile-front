...
export default function ConversationComposerModal({
  ...
  return (
    <div className="fixed inset-0 z-50">
      <div
        className={`absolute inset-0 backdrop-blur-sm transition-colors ${isDark ? "bg-black/60" : "bg-white/80"}`}
        onClick={() => {
          resetComposerFormState();
          onClose();
        }}
      ...
      <div className="absolute inset-0 overflow-y-auto">
        <div className="flex min-h-full items-start justify-center p-4 pb-8 sm:p-6 sm:pb-10">
          <div
            className={`my-4 w-full max-w-2xl overflow-hidden rounded-t-2xl lg:rounded-2xl border shadow-2xl transition-colors sm:my-6 ${isDark ? "border-white/40 bg-black" : "border-white/40 bg-white"}`}
          >
            {/* Header */}
            ...
            {/* Form View / Add Extra Members Toggle Wrapper */}
            {!showExtraMembers ? (
                      ...
                      <div className={`p-4 space-y-4`}>
                        <div className={`flex gap-2.5`}>
                          ...
                          <p className={`text-sm lg:text-base font-medium ${isDark ? "text-white" : "text-black"}`}>
                            Default Members <span className={isDark ? "text-[#E8D1AB]" : "text-black/80"}>(Add member count here)</span>
                          </p>
                        </div>
                          ...
                          {projectDetails?.client_name || projectDetails?.guest_email ? (
                            <div className={`flex items-center justify-between gap-3 rounded-md p-2.5 transition w-full min-w-0 ${isDark ? "bg-[#0C0C0C]" : "bg-white"}`}>
                              <div className="flex items-center gap-3 min-w-0 flex-1">
                                <div className="w-8 h-8 rounded-full bg-[#F5F5F5] overflow-hidden flex items-center justify-center text-black font-semibold text-sm relative shrink-0">
                                  {getInitials(projectDetails?.client_name || projectDetails?.guest_email)}
                                </div>
              ...
              <div className="p-6 border-t border-dashed border-white/10">
                ...
                <div className={`max-h-[460px] overflow-y-auto no-scrollbar border rounded-lg lg:rounded-xl ${isDark ? "border-white/20 bg-[#171717]" : "border-[#E3E3E3] bg-[#F4F5F7]"}`}>
            ...
            <div className={`p-4 lg:p-6 !pt-0 mt-auto flex gap-2.5 lg:gap-3 shrink-0`}>
              <button
                ...
                className={`lg:w-25 rounded-lg border p-3 lg:h-12 text-xs lg:text-sm font-medium transition-all ${isDark ? "border-[#262626] bg-[#1F1F1F] text-white hover:bg-[#1F1F1F]/80" : "border-[#f0f0f0] bg-[#f0f0f0] text-zinc-700 hover:bg-zinc-100"}`}
              >
                Cancel
              </button>
              <button
                ...
                onClick={submit}
                disabled={submitting}
                className={`lg:w-25 rounded-lg p-3 lg:h-12 text-xs lg:text-sm font-medium transition-all disabled:opacity-40 disabled:pointer-events-none bg-[#E8D1AB] text-black hover:bg-[#d4c2a1] flex gap-1 items-center`}
              >
