...
export default function Topbar({ pathname, actions, title, breadcrumbOverrides }: TopbarProps) {
    ...
    // Breadcrumb logic for creator routes
    const paths = pathname
        .split("/")
        .filter((path) => path)
        .filter((path) => path !== "creator" && path !== "dashboard");

    const isShootsPage = pathname.includes("request");
                    ...
                    {isShootsPage ? (
                        ...
                        <nav className={`flex items-center gap-2 text-xs whitespace-nowrap ${isDark ? "text-white/40" : "text-[#00000066]"}`}>
                            {paths.map((path, index) => {
                                ...
                                // Custom display text mapping for creator routes
                                const displayText = breadcrumbOverrides?.[path] ||
                                    (path === "request" ? "request & shoots" :
                                        path === "file-manager" ? "file manager" :
                                            path === "finance" ? "finances" :
                                                path === "payouts" ? "payouts" :
          ...
          ========================================== */}
            <div className="hidden lg:flex items-center justify-between px-9 py-6 gap-4">
                ...
                <div className="flex items-center gap-6 shrink-0">
                    {
                        isShootsPage ? (
                            <h1 className={`font-semibold text-lg ${isDark ? "text-white" : "text-[#101010]"}`}>{title || "Request & Shoots"}</h1>
                        ) : (
