...
export default function Step2Form({ data, setData, nextStep, prevStep }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!data.skills || data.skills.length === 0) {
     ...
     const isOnlyEditor = data.roles?.length === 1 && data.roles.includes("3");
    if (!isOnlyEditor && (!data.equipments || data.equipments.length === 0)) {
        ...
        <div className={sectionClasses}>
          <div>
            <h2 className="text-base font-semibold text-white">
              ...
              {data.roles?.length === 1 && data.roles.includes("3") ? (
